...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  if (dtUserTable) {
    ...
    dt_User = new DataTable(dtUserTable, {
      ajax: {
        // آدرس AJAX Controller برای لود داده‌های مشتریان
      ...
      columns: [
        ...
        { data: 'user_id' },
        { data: 'user_id', orderable: false, render: DataTable.render.select() },
        { data: 'user' },
        { data: 'current_plan' },
        { data: 'join_at' },
        { data: 'id' }
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            const name = full['user']['first_name']+ " " + full['user']['last_name'];
        ...
        },
        {
          targets: 3,
          render: function (data, type, full, meta) {
            return `<span class='text-truncate d-flex align-items-center'><span class="me-2"><i class="icon-base ti tabler-chart-pie icon-22px text-info"></i></span>${full['plan']['name']}</span>`;
          }
        },
        {
          targets: 4,
          render: function (data, type, full, meta) {
            return '<span class="fw-medium">' + full['join_at'] + '</span>';
          }
        },
        // {
        //   // User Status
