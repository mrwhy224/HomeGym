...
@section('content')
	<div class="invoice-print p-6">
		<div class="d-flex justify-content-between flex-row mb-6">
			...
			<div class="text-end">
				<h5 class="mb-2">Invoice #INV-{{ $invoice->id }}</h5>
			...
			<div class="col-6">
				...
				<table>
					...
					<tr>
						...
						<td class="fw-medium">{{ number_format($invoice->amount) }} {{ $invoice->currency->code }}</td>
					</tr>
					<tr>
		...
		<div class="table-responsive border rounded mb-6">
			<table class="table m-0">
				...
				<tbody>
				<tr>
					<td class="text-nowrap text-heading">{{ $invoice->invoiceable->package->name }}</td>
					<td>Online / Digital Session</td>
					<td>{{ $invoice->invoiceable->package->total_sessions }} Sessions</td>
					<td>{{ number_format($invoice->amount) }} {{ $invoice->currency->code }}</td>
					<td>{{ number_format($invoice->amount) }} {{ $invoice->currency->code }}</td>
				</tr>
				</tbody>
			</table>
				...
				<table class="table m-0 table-borderless">
					...
					<tr>
						...
						<td class="text-end px-0 py-6 w-px-100">
							<p class="fw-medium mb-2">{{ number_format($invoice->amount) }} {{ $invoice->currency->code }}</p>
							<p class="fw-medium mb-2">0</p>
							<p class="fw-bold mb-0 pt-2">{{ number_format($invoice->amount) }} {{ $invoice->currency->code }}</p>
						</td>
					</tr>
					</tbody>
				</table>
